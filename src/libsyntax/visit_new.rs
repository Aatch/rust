...
pub trait Visitor {
    ...
    pub fn visit_local_contents(&mut self, loc: &local) {
        ...
        match loc.node.init {
            None => (),
            Some(@ref ex) => self.visit_expr(ex)
        }
    }
    ...
    pub fn visit_pat_contents(&mut self, p: &pat) {
        match p.node {
          ...
          | pat_region(@ref inner) => self.visit_pat(inner),
            ...
            pat_lit(@ref ex) => self.visit_expr(ex),
            pat_range(@ref e1, @ref e2) => {
                self.visit_expr(e1);
                self.visit_expr(e2);
            }
            pat_wild => (),
    ...
    pub fn visit_ty_param_bounds_contents(&mut self, bounds: &OptVec<TyParamBound>) {
        for bounds.iter().advance |bound| {
            match bound {
                &TraitTyParamBound(ref ty) => self.visit_path(&ty.path),
                &RegionTyParamBound => ()
            }
        }
    }
    ...
    pub fn visit_expr_contents(&mut self, ex: &expr) {
        match ex.node {
            expr_vstore(@ref x, _) => self.visit_expr(x),
            expr_vec(ref es, _) => {
                for es.iter().advance |&ex| {
                    self.visit_expr(ex)
                }
            }
            expr_repeat(@ref element, @ref count, _) => {
                self.visit_expr(element);
                self.visit_expr(count);
            }
            expr_struct(ref p, ref flds, ref base) => {
                self.visit_path(p);
            ...
            expr_call(@ref callee, ref args, _) => {
                for args.iter().advance |&ex| {
                    self.visit_expr(ex);
                }
                self.visit_expr(callee);
            }
            expr_method_call(_, @ref callee, _, ref tys, ref args, _) => {
                for args.iter().advance |&ex| {
                    self.visit_expr(ex);
                }
            ...
            expr_while(@ref x, ref b) => {
                self.visit_expr(x);
                self.visit_block(b);
            }
            expr_loop(ref b, _) => self.visit_block(b),
            expr_match(@ref x, ref arms) => {
                self.visit_expr(x);
            ...
            expr_assign(@ref a, @ref b) => {
                self.visit_expr(b);
                self.visit_expr(a);
            }
            expr_copy(@ref a) => self.visit_expr(a),
            expr_assign_op(_, _, @ref a, @ref b) => {
                self.visit_expr(a);
                self.visit_expr(b);
            }
            ...
            expr_index(_, @ref a, @ref b) => {
                self.visit_expr(a);
                self.visit_expr(b);
            }
            expr_path(ref p) => self.visit_path(p),
            expr_self => (),
            expr_break(_) => (),
            expr_again(_) => (),
            expr_ret(Some(@ref e)) => self.visit_expr(e),
            expr_ret(_) => (),
            expr_log(@ref lv, @ref x) => {
                self.visit_expr(lv);
                self.visit_expr(x);
            }
            expr_mac(ref mac) => self.visit_mac(mac),
            expr_paren(@ref x) => self.visit_expr(x),
