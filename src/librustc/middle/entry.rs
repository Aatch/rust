            ...
            *self.session.entry_type = Some(session::EntryMain);
            ...
            if !*self.session.building_library {
                ...
                if !self.non_main_fns.is_empty() {
                    ...
                    self.session.note("the main function must be defined at the crate \
                                       level but you have one or more functions named \
                                       'main' that are not defined at the crate level. \
                                       Either move the definition or attach the `#[main]` \
                                       attribute to override self behavior.");
